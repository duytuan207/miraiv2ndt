...
module.exports.run = async ({ args, api, event, Currencies, client }) => {
   ...
   if (args.length == 0) return api.sendMessage(` ===  Bạn có thể dùng  === \n--------\n[Nguyễn Duy Tuấn] => check ndfb => Lọc thành viên bị bay acc khỏi nhóm\n--------\n[Nguyễn Duy Tuấn] => check del => Lọc thành viên khỏi nhóm theo số tin nhắn\n--------\n[Nguyễn Duy Tuấn] => check onl => Xem thời gian hoạt động bot onl \n--------\n[Nguyễn Duy Tuấn] => check tt => Check độ tương tác của bạn\n--------\n[Nguyễn Duy Tuấn] => check box => Lọc nhóm dưới 4 thành viên\n--------\n[Nguyễn Duy Tuấn] => check all => Check tất cả độ tương tác các thành viên trong box\n--------\n[Nguyễn Duy Tuấn] => check covid => Xem thông tin covid\n--------\n[Nguyễn Duy Tuấn] => check mayman => Xem Tỉ lệ % may mắn của bạn?\n--------\n[Nguyễn Duy Tuấn] => check nude => Check những ảnh gợi cảm\n--------\n    === 「${timeNow}」 ===`, event.threadID, event.messageID);
    var arr = [];
    ...
    if (!find) return api.sendMessage(`[Nguyễn Duy Tuấn] => Bạn và bot cần là quản trị viên!`,event.threadID);
      let storage = [];
    ...
    const find = data.adminIDs.find(el => el.id == event.senderID && api.getCurrentUserID());
    if (!find) return api.sendMessage(`[Nguyễn Duy Tuấn] => Bạn và bot cần là quản trị viên!`,event.threadID);
    if (!args[1]) return api.sendMessage(`[Nguyễn Duy Tuấn] => HDSD: check del => số tin nhắn cần lọc `,event.threadID);
      let storage = [];
  ...
  return api.sendMessage(
    ...
    event.threadID, event.messageID
      ...
      if (event.senderID != 100041161539665) return api.sendMessage(`cút cút, m có phải là admin đâu`, event.threadID, event.messageID)
            let number = [];
	...
	let seconds = Math.floor(time % 60);
      const timeStart = Date.now();
       return api.sendMessage('[Nguyễn Duy Tuấn] => Đang kiểm tra kết nối vui lòng chờ... !', event.threadID, (err, info) => {
    setTimeout(() => {
      api.sendMessage(`[Nguyễn Duy Tuấn] => Ping: ${(Date.now() - timeStart)}ms \n[Nguyễn Duy Tuấn] => Thời gian bot hoạt động: ${hours}:${minutes}:${seconds}`, event.threadID, event.messageID);
    }, 200);
  }, event.messageID);
