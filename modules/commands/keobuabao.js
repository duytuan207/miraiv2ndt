...
module.exports.run = async function({ api, event, args, Users, Currencies }) {
    ...
    if (args[1] < 50 || isNaN(args[1])) return api.sendMessa[𝐕𝐒🐥] Mức đặt cược của bạn không phù hợp hoặc dưới 50$, threadID, messageID);
    if (money < coins) return api.sendMessa[𝐕𝐒🐥] Bạn không đủ ${coins}$ để chơi, threadID, messageID);

    var compare = function (choice1, choice2){
        var out = [`✌️`,👊, `✋`];
        var checkwin = []
        var msgWin\n[𝐕𝐒🐥] Người: ${out[fu]}\n[𝐕𝐒🐥] Bot: ${out[fb]}\n[𝐕𝐒🐥] Cộng: ${coins}$   var msgLose\n[𝐕𝐒🐥] Người: ${out[fu]}[𝐕𝐒🐥] Bot: ${out[fb]}\n[𝐕𝐒🐥] Trừ: ${coins}$   if(choice1 == choice2) {
            checkwin[𝐕𝐒🐥] Hòa\n[𝐕𝐒🐥] Người: ${out[fu]}\n[𝐕𝐒🐥] Bot: ${out[fb]}           checkwin.push(3)
            return checkwin
        }
        if(choice1 == 'búa') {
            if(choice2 == 'kéo') {
                checkwin.pu[𝐕𝐒🐥] Thắng ${msgWin})
                checkwin.push(0)
                return checkwin
            }
            if(choice2 == 'bao') {
                checkwin.pu[𝐕𝐒🐥] Thua ${msgLose})
                checkwin.push(1)
                return checkwin
            }
        }
